...
def resize_image_to_match_reference(
    image_to_resize: np.ndarray, reference_image: np.ndarray
) -> np.ndarray:
    """
    Resize an image so that it matches the width and height of a reference image.
    Args:
        image_to_resize (np.ndarray): image that may need resizing.
        reference_image (np.ndarray): reference image whose size to match.
    Returns:
        np.ndarray: resized image (or original if already matching).
    """
